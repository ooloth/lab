...
class Range extends Component {
  ...
  render() {
    ...
    if (position && opponent.action && opponent.position) {
      currentActions =
        actions[position][`VERSUS_${opponent.action}`][opponent.position]
    } else if (position !== `bigBlind`) {
...
const Filters = ({
  ...
  if (position) {
    ...
    heroPositions = allPositions.filter(
      position => position.indexOf(`initial`) === -1
    )
  }
  ...
  return (
    ...
    >
      <div>
            ...
            {opponentActions.map(action => {
              return (
                <Filter
                  key={shortid.generate()}
                  onClick={() =>
                    updateOpponentAction(action.replace(/VERSUS_/g, ''))
                  }
                  className={`poker-link mh1 ${
                    opponent.action
        ...
        {opponentActions && opponentPositions && (
          <Fragment>
            <span>from the</span>
...
const actions = {
  ...
  hijack: {
    ...
    RFI: {
      raise: prange(
        `22+, A2s+, ATo+, K8s+, KJo+, Q9s+, QJo, J9s+, T8s-97s, T9s-54s`
      ),
  ...
  cutoff: {
    ...
    RFI: {
      raise: prange(
        `22+, A2s+, ATo+, K6s+, KTo+, Q8s+, QTo+, J8s+, JTo, T8s-64s, T9s-54s`
      ),
      raiseOrFold: prange(
    ...
    VERSUS_3BET: {
      button: {
        ...
        call: prange(
          `99-44, AQs-A9s, A5s, AQo-AJo, KQs-K9s, KQo, QJs-Q9s, J9s-T8s, JTs-76s`
        ),
  ...
  button: {
    VERSUS_RFI: {
      ...
      cutoff: {
        ...
        call: prange(
          `99-22, ATs-A8s, A5s-A4s, AJo, KTs-K9s, KQo, QTs-Q9s, J9s-97s, T9s-76s`
        ),
...
class PotOdds extends Component {
  ...
  render() {
    ...
    const equityRequiredToCall = Math.floor(
      (betSize / (betSize + betSize + 1)) * 100
    )
    ...
    return (
      <article className="flex items-baseline pl1 pv1">
        ...
        <p className="pl1">
          bet, I need <span className="f4 fw7">{equityRequiredToCall}%</span> equity
          to call.
        </p>
    ...
    return (
      <article className="sm:flex items-baseline pl1 pv2">
        ...
        <p className="pt1 sm:pt0">
          I need to win <span className="f4 fw7">{multipleOfBetsNeededToCall}x</span>{' '}
